...
* Description: Program 4 - otp_enc.c: Creates a client socket
*				and calls otp_enc_d to encrypt a message from
...
int main(int argc, char *argv[])
{
	...
	plainFD = open(argv[1], O_RDONLY);//file descriptor for plaintext file
	
	//get first char of plaintext
	read(plainFD, &buffer[i], 1);

	c = buffer[i]; //track characters coming in
	...
	//build string to send to otp_enc_d
	...
	//see if the server rejected the client
