...
* Description: Program 4 - otp_enc_d.c: Creates a server that
...
*				array and uses the key to encrypt the message
*				using one time pad method. It then sends the 
*				encrypted message back to the client.
**************************************************************/
	...
	while(1)
	{
		...
		switch(spawnPid)
		{
			...
			case 0: 
			{
				...
				//check client id
				if(strcmp(encID, "encode"))
				{
					//if not encode reject the client
				...
				else
				{
					...
					{
						//if char is a digit store it in length array
					...
					j = 6 + strlen(length); //encode id + length of message
					...
					for(i = 0; i < messLen; i++)
					{
						...
						c = (message[i] - 65) + (key[i] - 65);//convert ascii to alphabet values
						
						if(c < 0)
							c += 54; //convert negative value to work with mod operator
						c %= 27;
						c += 65;
						if(c == 91) //convert back to space
							c = 32;
						
						encMess[i] = c;
					}
					
					// Send a encoded message back to the client
